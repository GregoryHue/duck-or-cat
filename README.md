...
├── notebook/   # Jupyter notebook — trains & evaluates a custom CNN classifier
...
Run all cells. This trains a small CNN from scratch (no pretrained weights) on
`data/raw/`, evaluates it on a held-out test split, and writes `models/model.pt` +
`models/classes.json`.
